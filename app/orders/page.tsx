                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-6 animate-in fade-in slide-in-from-top-2 duration-300">
                    {dateBills.map((bill: any) => (
                      <Link 
                        key={bill._id} 
                        href={`/bill/${bill._id}`}
                        className="group block bg-white p-3.5 sm:p-5 rounded-[28px] sm:rounded-[36px] shadow-sm border border-slate-100 hover:border-teal-300 hover:shadow-xl hover:translate-y-[-4px] transition-all duration-300"
                      >
                        <div className="flex flex-col space-y-3 sm:space-y-4">
                          <div className="flex justify-between items-start">
                            <div className="flex items-center space-x-2 sm:space-x-3 min-w-0">
                                <div className="relative shrink-0">
                                  <div className="w-8 h-8 sm:w-11 sm:h-11 p-2 sm:p-2.5 bg-teal-50 text-teal-600 rounded-lg sm:rounded-2xl group-hover:bg-teal-600 group-hover:text-white transition-colors duration-300">
                                    <User className="w-full h-full" />
                                  </div>
                                  {bill.orderNumber && (
                                    <div className="absolute -top-1 -right-1 bg-teal-600 text-white text-[7px] sm:text-[9px] font-black px-1 py-0.5 rounded-md sm:rounded-lg shadow-sm border border-white">
                                      #{bill.orderNumber}
                                    </div>
                                  )}
                                </div>
                                <div className="min-w-0">
                                  <h3 className="font-black text-slate-800 text-[10px] sm:text-sm group-hover:text-teal-700 transition-colors uppercase tracking-tight truncate pr-0.5">
                                    {bill.customerName}
                                  </h3>
                            ...
                            <div className="shrink-0 text-right ml-1 sm:ml-2">
                              <p className="text-xs sm:text-lg font-black text-slate-900 tracking-tighter">₹{bill.grandTotal}</p>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-1 text-[8px] sm:text-[10px] text-slate-400 font-bold uppercase">
                              <Calendar size={10} className="text-teal-500 sm:w-3 sm:h-3" />
                              <span>{new Date(bill.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                            </div>
                            <div className="flex items-center space-x-0.5 sm:space-x-1 py-0.5 sm:py-1 px-1.5 sm:px-3 bg-slate-50 rounded-lg sm:rounded-xl group-hover:bg-teal-50 group-hover:text-teal-700 transition-all text-[8px] sm:text-[10px] font-black uppercase text-slate-500">
                              <span>{bill.items.length} ITM</span>
                              <ChevronRight size={8} className="sm:w-2.5 sm:h-2.5 group-hover:translate-x-0.5 transition-transform" />
                            </div>
                          </div>
                        </div>
                      </Link>
