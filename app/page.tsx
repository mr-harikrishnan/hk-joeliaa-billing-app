    ...
    <div className="space-y-8 max-w-6xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-end md:justify-between space-y-4 md:space-y-0">
        <div>
          ...
          <p className="text-slate-500 font-medium">Monitoring your homemade treats business performance</p>
        </div>
        <div className="flex items-center space-x-4 bg-white p-2 pr-6 rounded-2xl shadow-sm border border-slate-100">
          ...
          <div className="hidden sm:block">
            ...
            <span className="text-[10px] font-bold text-teal-600 uppercase tracking-widest">Active Store</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {cards.map((card) => (
          <div key={card.title} className="bg-white p-6 rounded-[32px] shadow-sm border border-slate-100 flex flex-col justify-between group hover:shadow-md transition-all duration-300">
            ...
            <div className="mt-4 pt-4 border-t border-slate-50 flex items-center justify-between">
              <span className="text-xs font-semibold text-slate-500">{card.sub}</span>
              <ChevronRight className="text-slate-300 group-hover:text-teal-500 transition-colors" size={16} />
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-teal-600 p-8 rounded-[40px] shadow-xl shadow-teal-100 relative overflow-hidden flex flex-col justify-between">
          <div className="relative z-10">
            ...
            <h3 className="text-white font-black text-2xl leading-tight">Generate<br />Instant Bill</h3>
            <p className="text-teal-100 text-sm mt-3 font-medium opacity-80">Quick checkout for your customers</p>
          </div>
          <button 
